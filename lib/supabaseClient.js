...
// Anlaşmalar (örn. "Sucu ile 700.000₺'ye anlaştık") — bir borç değil, bir hizmet/iş
// için üzerinde anlaşılan toplam tutar ve buna karşılık yapılan kısmi ödemelerin defteri.
export function agreementPaidAmount(agreement, payments) {
  return payments
    .filter((p) => p.agreement_id === agreement.id)
    .reduce((s, p) => s + Number(p.amount), 0);
}

export function agreementRemaining(agreement, payments) {
  const remaining = Number(agreement.total_amount) - agreementPaidAmount(agreement, payments);
  return remaining > 0.0001 ? remaining : 0;
}

export function isAgreementSettled(agreement, payments) {
  return agreementRemaining(agreement, payments) <= 0.0001;
}
